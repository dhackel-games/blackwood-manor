#!/bin/bash
# Play Blackwood Manor.command. Copyright (c) dhackel-games. All Rights Reserved. 2026...2026-09-12.067:acoven.
...
# Reuse our no-cache server, otherwise replace any legacy http.server process
# and start a fresh one on a free port.
PIDF=".server.pid"
PID=""
[ -f "$PIDF" ] && PID=$(cat "$PIDF")
SERVER_COMMAND=""
if [ -n "$PID" ] && kill -0 "$PID" 2>/dev/null; then
  SERVER_COMMAND=$(ps -p "$PID" -o command=)
fi
if printf '%s' "$SERVER_COMMAND" | grep -q 'no-cache-server.py'; then
  PORT=$(printf '%s' "$SERVER_COMMAND" | sed -n 's/.*no-cache-server.py \([0-9][0-9]*\).*/\1/p')
else
  if printf '%s' "$SERVER_COMMAND" | grep -q 'python.*-m http.server'; then kill "$PID"; fi
  PORT=$("$PY" -c 'import socket;s=socket.socket();s.bind(("127.0.0.1",0));print(s.getsockname()[1]);s.close()')
  nohup "$PY" tools/no-cache-server.py "$PORT" >/dev/null 2>&1 &
  echo $! > "$PIDF"
  sleep 1
fi
